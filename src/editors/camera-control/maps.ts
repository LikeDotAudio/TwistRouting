// src/editors/camera-control/maps — the robotics studio maps (top-down + side
// elevation) and their per-frame update. Port of js/editors/camera/maps.js.

import type { CamState } from './state.js';

export function topSVG(): string {
...
export function sideSVG(): string {
  return `<svg viewBox="0 0 300 290" preserveAspectRatio="xMidYMid meet">
...
export function updateMaps(body: HTMLElement, s: CamState): void {
  const cam = body.querySelector('#cc-tcam');
  const cone = body.querySelector('#cc-tcone');
  if (cam && cone) {
    const x = 40 + s.dolly * 220;
    const y = 250 - s.ped * 30;
    const ang = (s.pan - 0.5) * 150;
    const half = 14 + (1 - s.zoom) * 34;
    const len = 120 + s.zoom * 80;
    cam.setAttribute('transform', `translate(${x},${y})`);
    cone.setAttribute('transform', `rotate(${ang})`);
    const a = (half * Math.PI) / 180;
    cone.setAttribute(
      'points',
      `0,0 ${Math.sin(-a) * len},${-Math.cos(-a) * len} ${Math.sin(a) * len},${-Math.cos(a) * len}`,
    );
  }
  const scam = body.querySelector('#cc-scam');
  const scone = body.querySelector('#cc-scone');
  if (scam && scone) {
    const x = 40 + s.dolly * 150;
    const y = 250 - s.ped * 150;
    const tilt = (s.tilt - 0.5) * 90;
    const half = 12 + (1 - s.zoom) * 26;
    const len = 150 + s.zoom * 90;
    scam.setAttribute('transform', `translate(${x},${y})`);
    scone.setAttribute('transform', `rotate(${tilt})`);
    const a = (half * Math.PI) / 180;
    scone.setAttribute(
      'points',
      `0,0 ${Math.cos(-a) * len},${Math.sin(-a) * len} ${Math.cos(a) * len},${Math.sin(a) * len}`,
    );
  }
}
